...
if [ ${#DATABASE} -ge 5 ]; then
    ...
    echo "Run Database container server name '${DATABASE_NAME}' and docker Network '${NETWORK_NAME}' first";
...
docker run --hostname $SERVICE_NAME --user "$(id -u):$(id -g)" -it --network bluebasket-net --name $SERVICE_NAME $PORT_ADDRESS $ADDITIONAL_VOLUMES -v ${PROJECT_PWD}/../:${PROJECT_PWD}/../ "${SERVICE_IMAGE}:latest" /bin/bash;
